...
with app.app_context():
    ...
    current_date = datetime(2023,11,9).date()
    ...
    for team_player_name, team_player_odds in new_odds_dict.items():
        ...
        if any(player_team in game.values() for game in todays_games):
            ...
            if len(uniq_game_list) > 0:
                ...
                assists_predict = round(0.6*assists_factor+0.4*assists_similar,2)
                points_predict = round(0.6*points_factor+0.4*points_similar,2)
                trb_predict = round(0.6*trb_factor+0.4*trb_similar,2)
